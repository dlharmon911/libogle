...
int32_t ogle_mesh_data_loader(o_mesh_data_t* object, void* data)
{
	...
	const o_mesh_data_file_data_t* file_data = (o_mesh_data_file_data_t*)data;

	if (!file_data->m_filename)
	{
		OGLE_DO_LOG(OGLE_LOG_LEVEL_ERROR, "Mesh data filename is NULL");
		return -1;
	}

	switch (file_data->m_type)
	{
	case OGLE_MESH_DATA_FILE_FORMAT_BIN:
	{
		if (ogle_mesh_data_load_bin(file_data->m_filename, object) < 0)
		{
			OGLE_DO_LOG(OGLE_LOG_LEVEL_ERROR, "Failed to load mesh data from binary file: %s", file_data->m_filename);
			return -1;
		}
		break;
	}
	case OGLE_MESH_DATA_FILE_FORMAT_OBJ:
	{
		if (ogle_mesh_data_load_obj(file_data->m_filename, object) < 0)
		{
			OGLE_DO_LOG(OGLE_LOG_LEVEL_ERROR, "Failed to load mesh data from OBJ file: %s", file_data->m_filename);
			return -1;
		}
		break;
	}
	default:
	{
		OGLE_DO_LOG(OGLE_LOG_LEVEL_ERROR, "Unsupported mesh data file format: %d", file_data->m_type);
		return -1;
	}
	}
